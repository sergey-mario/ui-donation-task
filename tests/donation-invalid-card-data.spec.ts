...
test.describe('Donation', async () => {
    test('User can not make donation if enter invalid card data', async ({
        clickMePage,
        secureDonationPage,
        paymentOptionPage,
        creditCardPage,
        ...
        await personalInformationPage
            .enterPersonalData(faker.person.firstName(), faker.person.lastName(), faker.internet.email());
